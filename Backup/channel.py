...
class ApplyOFDMChannel(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        x = expand_to_rank(
            x, h_freq.shape.rank, axis=1
        )  # [64, 1(added), 1(added), 1, 1, 14, 76]
        y = tf.reduce_sum(
            tf.reduce_sum(h_freq * x, axis=4), axis=3
        )  # [64, 1, 1, 14, 76] (3,4 removed)
...
def cir_to_ofdm_channel(frequencies, a, tau, normalize=False):
    ...
    e = tf.exp(tf.complex(tf.constant(0, real_dtype), -2 * PI * frequencies * tau))
    h_f = h * e
    ...
    if normalize:
        ...
        # subcarriers.
        c = tf.reduce_mean(tf.square(tf.abs(h_f)), axis=(2, 4, 5, 6), keepdims=True)
        c = tf.complex(tf.sqrt(c), tf.constant(0.0, real_dtype))
        h_f = tf.math.divide_no_nan(h_f, c)

    return h_f


def subcarrier_frequencies(num_subcarriers, subcarrier_spacing, dtype=tf.complex64):
    # pylint: disable=line-too-long
    r"""
    ...
    frequencies = tf.range(start=start, limit=limit, dtype=real_dtype)
...
def cir_to_time_channel(bandwidth, a, tau, l_min, l_max, normalize=False):
    ...
    if len(tau.shape) == 4:
        ...
        #  - number of tx antennas (4)
        tau = tf.expand_dims(
            tf.expand_dims(tau, axis=2), axis=4
    ...
    g = tf.complex(g, tf.constant(0.0, real_dtype))
    ...
    if normalize:
        ...
        c = tf.reduce_mean(
            tf.reduce_sum(tf.square(tf.abs(hm)), axis=6, keepdims=True),  # sum for time
            axis=(2, 4, 5),
            keepdims=True,
        )
        c = tf.complex(tf.sqrt(c), tf.constant(0.0, real_dtype))
...
class ApplyTimeChannel(tf.keras.layers.Layer):
    ...
    def __init__(
        self, num_time_samples, l_tot, add_awgn=True, dtype=tf.complex64, **kwargs
    ):
    ...
    def call(self, inputs):
        ...
        x = insert_dims(
            x, 2, axis=1
        )  # inserts a new dimension into the x tensor (64, 1, 1, 1065)
...
class GenerateOFDMChannel:
    ...
    def __init__(
        self, channel_model, resource_grid, normalize_channel=False, dtype=tf.complex64
    ):
        ...
        self._sampling_frequency = 1.0 / resource_grid.ofdm_symbol_duration  # 60000

        # Frequencies of the subcarriers
        self._frequencies = subcarrier_frequencies(
            self._num_subcarriers, self._subcarrier_spacing, dtype
        )  # [76]
    ...
    def __call__(self, batch_size=None):
        ...
        h, tau = self._cir_sampler(
            batch_size, self._num_ofdm_symbols, self._sampling_frequency  # 14
        )  # 60000
...
class NearestNeighborInterpolator(BaseChannelInterpolator):
    ...
    def _interpolate(
        ...
        outputs = tf.gather(
            inputs, self._gather_ind, 2, batch_dims=2
        )  # inputs: (1, 2, 128, 2, 1, 16) _gather_ind: (1, 2, 14, 64)
...
class LinearInterpolator(BaseChannelInterpolator):
    ...
    def __init__(self, pilot_pattern, time_avg=False):
        ...
        ##
        self._x_freq = tf.cast(
            expand_to_rank(tf.range(0, mask.shape[-1]), 7, axis=0), pilots.dtype
        )
        ...
        self._x_time = tf.cast(
            expand_to_rank(self._x_time, 7, axis=0), dtype=pilots.dtype
        )
        ...
        self._x_0_time = expand_to_rank(
            tf.expand_dims(self._y_0_time_ind, -1), 7, axis=0
        )
        self._x_0_time = tf.cast(self._x_0_time, dtype=pilots.dtype)
        self._x_1_time = expand_to_rank(
            tf.expand_dims(self._y_1_time_ind, -1), 7, axis=0
        )
    ...
    def _interpolate(self, inputs):
        ...
        # [k, l, m, num_tx, num_streams_per_tx, num_ofdm_symbols,...
        #  ...num_effective_subcarriers]
        h_hat_time = self._interpolate_1d(
            h_hat_time,
            self._x_time,
            self._x_0_time,
            self._x_1_time,
...
class MyLSChannelEstimator:
    ...
    def __init__(
        ...
        **kwargs
        ...
        print(
            self._pilot_ind[0, 0, :]
        )  # (1, 2, 128) int32 only 128 pilots [128-191 704-767]
    ...
    def estimate_at_pilot_locations(self, y_pilots, no):
        ...
        # Broadcasting from pilots here is automatic since pilots have shape
        # [num_tx, num_streams, num_pilot_symbols]
        h_ls = tf.math.divide_no_nan(
            y_pilots, self._pilot_pattern.pilots
        )  # pilots: (1, 2, 128)=>(2, 1, 16, 1, 2, 128)
        ...
        # Expand rank of pilots for broadcasting
        pilots = expand_to_rank(
            self._pilot_pattern.pilots, tf.rank(h_ls), 0
        )  # (1, 2, 128)=>(1, 1, 1, 1, 2, 128)
    ...
    def __call__(self, inputs):
        ...
        #  ..., num_pilot_symbols]
        y_pilots = tf.gather(
            y_eff_flat, self._pilot_ind, axis=-1
        )  # y_eff_flat:(b, 1, 16, 896) pilot_ind: (1, 2, 128) =>(b, 1, 16, 1, 2, 128)
        ...
        h_hat, err_var = self.estimate_at_pilot_locations(
            y_pilots, no
        )  # h_hat: (2, 1, 16, 1, 2, 128), err_var: (1, 1, 1, 1, 2, 128)
        ...
        if self._interpolation_type is not None:
            h_hat, err_var = self._interpol(
                h_hat, err_var
            )  # h_hat: (2, 1, 16, 1, 2, 128)=>(2, 1, 16, 1, 2, 14, 64)
            np.save("data/h_hat_inter2.npy", h_hat.numpy())

            err_var = tf.maximum(
                err_var, tf.cast(0, err_var.dtype)
            )  # (1, 1, 1, 1, 2, 14, 64)=>(1, 1, 1, 1, 2, 14, 64)
...
class GenerateFlatFadingChannel:
    ...
    def __init__(
        self, num_tx_ant, num_rx_ant, spatial_corr=None, dtype=tf.complex64, **kwargs
    ):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._gen_chn = GenerateFlatFadingChannel(
            self._num_tx_ant, self._num_rx_ant, spatial_corr, dtype=dtype
        )
...
class CIRDataset(ChannelModel):
    ...
    def __init__(
        ...
        output_signature = (
            ...
            tf.TensorSpec(shape=[num_rx, num_tx, num_paths], dtype=dtype.real_dtype),
        )
        dataset = tf.data.Dataset.from_generator(
            cir_generator, output_signature=output_signature
        )  # specifies the expected output signature of the generator function
    ...
    def __call__(self, batch_size=None, num_time_steps=None, sampling_frequency=None):
